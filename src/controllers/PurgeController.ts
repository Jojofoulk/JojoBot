...
export class PurgeController {
    ...
    purge(message: Message) {
        //+1 if we don't want to count the command itself, thus deleting outside of the count (don't forget the -1 in the print message to compensate)
        message.channel.bulkDelete(this.purgeSize + 1)
        .then((m)=>{
            DiscordUtils.displayText(message, `Deleted **${m.size-1}** messages`)
        })
        .catch((e: DiscordAPIError) => {
            //These are common errors for bulkDelete. A more generic handling for generic DiscordAPI error can be made later on
            if(e.code === 50034) {
